...
function Navbar() {
    ...
    //for theme and saving in local storage
    useEffect(()=>{
      if(theme==="dark"){
  ...
  <div className="navbar-start">
    <div className="dropdown">
      ...
      <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 dark:bg-slate-800 dark:text-white">{navItems}</ul>
    </div>
    <a className="btn btn-ghost text-2xl font-bold cursor-pointer">AuraBooks</a>
  </div>
  <div className="navbar-end space-x-3">
  
  <div className="navbar-center hidden lg:flex">
    <ul className="menu menu-horizontal px-3 space-x-10">{navItems}</ul>

  </div>
  {/* <div className="hidden md:block">
    <label className="px-3 py-2 border rounded-md flex items-center gap-2">
    <input type="text" className={`grow outline-none dark:bg-slate-800 dark:text-white ${sticky? "grow outline-none bg-base-200":""}`} placeholder="Search" />
  ...
  onClick={()=>setTheme(theme==="dark" ? "light" : "dark")}>
...

    {
      ...
      <a className="btn bg-pink-500 text-white rounded-lg dark:border-none hover:scale-105  hover:text-black hover:bg-pink-300" onClick={()=> document.getElementById("my_modal_3").showModal()}>Login</a>
      <Login/>
    </div>
    }
  </div>
...
//showmodal is fnc is used to open thatg modal
